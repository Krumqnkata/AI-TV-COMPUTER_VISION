...
class TTSManager:
    ...
    def _generate_ai_joke(self, name):
        """ Генерира шега чрез Gemini AI с рандомизирани стилове """
        if not self.ai_enabled:
            return None
        
        # Различни стилове/настроения, от които AI да избира всеки път
        stranger_styles = [
            "Като саркастичен робот, на който му е омръзнало да вижда хора.",
            "Като параноичен детектив – питай го дали не е шпионин.",
            "Като строг охранител на дискотека, който се чуди дали да го пусне.",
            "Като учен, който току-що е открил нов, странен биологичен вид.",
            "Изключително драматично и философски – кой е той и накъде отива?"
        ]
        
        known_styles = [
            "Леко хапливо и иронично, в стил 'тъкмо си починах от теб и пак дойде'.",
            "Супер ентусиазирано, сякаш в стаята току-що е влязла рок звезда.",
            "Като коментатор на спортен мач, който отразява влизането му.",
            "Сякаш току-що си го хванал да прави нещо нередно или да закъснява.",
            "Мистериозно – намекни, че знаеш някаква негова тайна."
        ]

        if name == "Непознат":
            style = random.choice(stranger_styles)
            prompt = (
                f"Роля: {style} "
                "Действие: Пред камерата току-що застана напълно непознат човек. "
                "Задача: Напиши една ЕДИНСТВЕНА, много кратка и забавна закачка/реплика към него на български. "
                "ПРАВИЛА (АБСОЛЮТНО ЗАДЪЛЖИТЕЛНИ): "
                "1. Максимум 1 кратко изречение. "
                "2. БЕЗ НИКАКВИ въведения, обяснения или кавички. "
                "3. БЕЗ ЕМОДЖИТА (защото аудио софтуерът не може да ги чете)."
            )
        else:
            style = random.choice(known_styles)
            prompt = (
                f"Роля: {style} "
                f"Действие: Човек на име {name} току-що влезе в стаята и камерата го разпозна. "
                "Задача: Напиши една ЕДИНСТВЕНА, оригинална и много кратка шега/закачка към него на български. "
                "ПРАВИЛА (АБСОЛЮТНО ЗАДЪЛЖИТЕЛНИ): "
                "1. Максимум 1 кратко изречение. "
                "2. БЕЗ НИКАКВИ въведения, кавички или текст преди/след шегата. "
                "3. БЕЗ ЕМОДЖИТА (защото аудио софтуерът не може да ги чете)."
            )
        ...
        try:
            ...
            if response and response.text:
                ...
                # Премахване на случайни кавички и звездички
                joke = joke.replace('"', '').replace('*', '') 
                log_system(f"AI generated joke for {name} (Style: {style[:20]}...)")
                return joke
        except Exception as e:
