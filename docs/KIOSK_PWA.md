...
- лични имена и съобщения не се пазят в browser storage или Service Worker
  cache;
- TTS се стартира от видимия бутон **Прочети на глас** или от изрично
  включената администраторска настройка за автоматично прочитане;
...
киоска** определя дали полученият отговор да бъде прочетен веднага. Тя е
изключена по подразбиране и се прилага при следващия зададен въпрос, без
рестарт на устройството. Нов въпрос, редакция на полето, приключване, idle
timeout, pause, скриване или смяна на сесията прекъсват текущото четене.
Автоматичното четене изисква наличен Web Speech API в браузъра.
...
- автоматично TTS прочитане и прекъсване при нов въпрос/край на сесия;
